<template>
  <div>
    ...
    <button @click="add2">添加0.5</button>
    <button @click="add3">添加0</button>
...
export default {
  ...
  methods: {
    ...
    add1() {
      this.predictor.addImage(1);
    },
    add2() {
      this.predictor.addImage(0.5);
    },
    add3() {
      this.predictor.addImage(0);
    },
    train() {
    ...
    predict() {
      this.predictor.predict((err, result) => {
        if (err) {
          console.log(err);
        } else {
          this.predict()
